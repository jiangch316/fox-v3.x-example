...
export let options = (env = 'production') => {
  ...
  let options: RollupOptions = {
    input: join(__dirname, '../src/core/index.ts'),
    output: {
      file: join(__dirname, '../src/assets/libs/fox-v3.0.0/index.js'),
      format: 'esm',
      name: 'fox',
...
export let dtsOptions = () => {
  const options: RollupOptions = {
    ...
    input: join(__dirname, '../src/core/index.ts'),
    output: {
      file: join(__dirname, '../src/assets/libs/fox-v3.0.0/index.d.ts'),
      format: 'es',
    },
    plugins: [dts()],
    external: ['vue'],
